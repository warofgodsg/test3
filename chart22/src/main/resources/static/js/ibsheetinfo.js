 ...
 * 
 * 2021.08.30
 * 제우스의 input항목에 % 를 입력 후 저장을 하면 해당 화면의 IBSheet 데이터 조회 전 스크립트 오류가 발생 됨
 * 	- 제우스 에러 : java.lang.IllegalArgumentException: Invalid URL encoding: not a valid digit:94
 *  - 크롬 에러 : Unable to decode value
 *  - 처리방법 : .replace(/%/g,"%25") 추가
 * 		for (i = 0; i < j; i++) {
        	if (name[i] != '') plain_text += name[i] + "=" + value[i].replace(/&/g,"%26").replace(/\+/g,"%2B").replace(/%/g,"%25") + "&";
   		}
 */
...
function createIBSheet(sheetid, width, height, locale) {
    ...
    if (height == "100%" || height == "99%") height = "98%"; // 해상도가 작은 경우 그리드Height가 계속 늘어남...
    ...
    setConfigIBSheet(sheetid);
}

function setConfigIBSheet(sheetid){
    // 자동Trim 사용안함
    //eval(sheetid).SetDataAutoTrim(false);
    
    // Alternate 편집가능여부와 상관없이 무조건 표시....
    eval(sheetid).SetEditableColorDiff(0);
    
    // 자동편집상태
    //eval(sheetid).SetFocusEditMode(1);
    
    // 자동포커스
    //eval(sheetid).SetFocusAfterProcess(1);
    
    // Combo 항목이 없는 경우 항목 없음 표시하기
    eval(sheetid).InitComboNoMatchText(1, "");
	
    // 기본 상태를 한글 입력 상태로 함
    //alert(eval(sheetid).GetBasicImeMode());
    eval(sheetid).SetBasicImeMode(1); // 0:마지막상태, 1:한글, 2:영문

    eval(sheetid).SetComboOpenMode(1); 
    
    var cfg= {Alternate:1};
    eval(sheetid).SetConfig(cfg);
}
/*
